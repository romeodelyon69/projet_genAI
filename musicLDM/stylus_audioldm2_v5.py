"""
Stylus-AudioLDM2 : Music Style Transfer via AudioLDM2  — v5
=========================================================================
Adaptation of "Stylus: Repurposing Stable Diffusion for Training-Free
Music Style Transfer" (arxiv:2411.15913) for AudioLDM2.

Fixes v5 vs v4:
  1. Removed _match_shape: attention natively handles Q(Nq) × K(Nk)
     asymmetrically. Zero-pad/truncation was corrupting the softmax.
  2. AdaIN back to dim=[2,3]: VAE channels are abstract learned features,
     normalizing over T×F transfers the style distribution per channel
     (as in classical AdaIN and the original Stylus implementation).

Fixes inherited from v4:
  - Mel preprocessing aligned with AudioLDM2 training (hop=160, fmax=8000)
  - T5 projection: full sequence
  - Vocoder decode: official pipe.mel_spectrogram_to_waveform()
  - Attention processor compatible with diffusers v1/v2
  - Roundtrip verification: encode→decode and inversion→reverse
  - Targeting up_blocks layers [2,3]
"""
        ...
        if is_self:
            if store.mode == "capture_style":
                ...
                if ks is not None and qc is not None:
                    # Q_content and Q_current have the same N (same audio).
                    # K_style/V_style may have a different N if
                    # style and content have different durations:
                    # this is normal, attention handles Q(Nq,D) × K(Nk,D)^T
                    # natively without padding or truncation.
...
# ─────────────────────────────────────────────────────────────────────────────
# AdaIN — normalization over spatial dimensions [2,3] (T_lat × F_lat)
# ─────────────────────────────────────────────────────────────────────────────
...
def adain_latent(z_content: torch.Tensor, z_style: torch.Tensor) -> torch.Tensor:
    ...
    The C channels of the VAE are abstract learned features, not
    interpretable frequencies. To transfer the "style" (timbre),
    we align the statistical distribution (mean, std) of each channel
    over the spatial dimensions [2,3] = (T_lat, F_lat).

    This is identical to classical AdaIN for image style transfer
    (Huang & Belongie 2017) and the original Stylus implementation.
    """
    eps = 1e-5
...
class StylusAudioLDM2Pipeline:
    ...
    # ── [FIX #5] Reconstruction verification ───────────────────────────────────── ─────────────────────────────
...
def main():
    cfg = StylusAudioLDM2Config(
        ...
        style_audio_path="musicTI_dataset/audios/timbre/accordion/accordion1.wav",
        content_audio_path="musicTI_dataset/audios/content/color/color1.wav",
    ...
    print("=" * 60)
    print("  Stylus-AudioLDM2 — Music Style Transfer  (v5)")
