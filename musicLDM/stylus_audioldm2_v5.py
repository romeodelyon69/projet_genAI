...
Corrections v5 vs v4 :
  1. Suppression de _match_shape : l'attention gère nativement Q(Nq) × K(Nk)
     asymétrique. Le zero-pad/truncation corrompait le softmax.
  2. AdaIN revient à dim=[2,3] : les canaux VAE sont des features abstraites,
     normaliser sur T×F transfère la distribution de style par canal
     (comme dans l'AdaIN classique et l'implémentation Stylus originale).

Corrections héritées de v4 :
  - Mel preprocessing aligné sur AudioLDM2 training (hop=160, fmax=8000)
  - Projection T5 : séquence complète
  - Vocoder decode : pipe.mel_spectrogram_to_waveform() officiel
  - Attention processor compatible diffusers v1/v2
  - Vérification roundtrip encode→decode et inversion→reverse
  - Ciblage couches up_blocks [2,3]
"""
        ...
        if is_self:
            if store.mode == "capture_style":
                ...
                if ks is not None and qc is not None:
                    # Q_content et Q_current ont le même N (même audio).
                    # K_style/V_style peuvent avoir un N différent si
                    # style et content ont des durées différentes :
                    # c'est normal, l'attention gère Q(Nq,D) × K(Nk,D)^T
                    # nativement sans padding ni troncation.
...
# ─────────────────────────────────────────────────────────────────────────────
# AdaIN — normalisation sur les dimensions spatiales [2,3] (T_lat × F_lat)
# ─────────────────────────────────────────────────────────────────────────────
...
def adain_latent(z_content: torch.Tensor, z_style: torch.Tensor) -> torch.Tensor:
    ...
    Les canaux C du VAE sont des features abstraites apprises, pas
    des fréquences interprétables. Pour transférer le "style" (timbre),
    on aligne la distribution statistique (mean, std) de chaque canal
    sur les dimensions spatiales [2,3] = (T_lat, F_lat).

    C'est identique à l'AdaIN classique du style transfer d'images
    (Huang & Belongie 2017) et à l'implémentation originale de Stylus.
    """
    eps = 1e-5
