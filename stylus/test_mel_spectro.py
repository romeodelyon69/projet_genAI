...
Runs two style transfers with extreme parameters and compares
the resulting mel spectrograms:

  Config A: alpha=0, gamma=1  → pure content (reference "no style")
             - alpha=0: out = out_content (K/V style ignored)
             - gamma=1: Q = Q_content (maximum structure)

  Config B: alpha=1, gamma=0  → pure style (maximum injection)
             - alpha=1: out = out_style  (K/V content ignored)
             - gamma=0: Q = Q_current   (free query, no preservation)

If the pipeline works, the two mels must be VISUALLY different:
  - Config A ≈ mel_content (same structure, no style texture)
  - Config B ≈ style texture applied to content structure

The script produces:
  ./test_outputs/mel_A_alpha0_gamma1.png
  ./test_outputs/mel_B_alpha1_gamma0.png
  ./test_outputs/mel_comparison_AB.png   ← both side by side + diff
  ./test_outputs/diff_stats.txt          ← difference statistics
"""
...
def run_transfer(style_audio, content_audio, alpha, gamma, label, shared_pipeline=None):
    ...
    if shared_pipeline is None:
        ...
        # Reuse the loaded model, just update alpha/gamma
...
def save_comparison(mel_c, mel_s, mel_A, mel_B, cfg_audio, out_path):
    """
    4 panels: Content | Style | Config A (alpha=0,γ=1) | Config B (alpha=1,γ=0)
    + absolute difference panel A-B normalized
    """
    ...
    panels = [
        (axes[0, 0], mel_c, "Content (reference)"),
        (axes[0, 1], mel_s, "Style (reference)"),
        (axes[0, 2], mel_A, "Config A: α=0, γ=1\n(pure content — no injection)"),
        (axes[1, 0], mel_B, "Config B: α=1, γ=0\n(pure style — maximum injection)"),
    ]
    ...
    im_diff = ax_diff.imshow(
        ...
        aspect="auto",
        cmap="hot",
        extent=[0, dur_diff, fmin, fmax],
        interpolation="nearest",
    )
    ax_diff.set_title(
        "Absolute difference |A − B| (dB)\n→ red zones = injection had an effect",
        fontsize=11,
        fontweight="bold",
    )
    ...
    # Pearson correlation between A and B (on flattened mels)
    flat_A = mel_A[:, :T].flatten()
    flat_B = mel_B[:, :T].flatten()
    corr = np.corrcoef(flat_A, flat_B)[0, 1]

    # Correlation A vs content (to check if A ≈ content)
    flat_C = (
        mel_c[:, :T].flatten()
        if mel_c.shape[1] >= T
        else np.pad(mel_c.flatten(), (0, T * mel_c.shape[0] - len(mel_c.flatten())))
    )
    corr_A_C = (
        np.corrcoef(flat_A, flat_C[: len(flat_A)])[0, 1]
        if len(flat_A) == len(flat_C)
        else float("nan")
    )

    stats_text = (
        f"Difference statistics |A − B|\n"
        f"{'─' * 35}\n"
        f"  Mean       : {mean_diff:+.2f} dB\n"
        ...
        f"Interpretation\n"
        f"{'─' * 35}\n"
        f"  If corr(A,B) ≈ 1 → injection has no effect\n"
        f"  If corr(A,B) < 0.95 → injection visible\n"
        f"  If corr(A,C) > corr(A,B) → A ≈ content ✓"
    )
    ax_stats.text(
        0.05,
        0.95,
    ...
    plt.suptitle(
        "Injection test: impact of α and γ on the mel spectrogram",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
...
def main():
    ...
    # Base config to load model once
