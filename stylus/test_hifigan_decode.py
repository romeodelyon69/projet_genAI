...
Tests the encode → mel → HiFi-GAN → audio chain in isolation,
with Stylus mel parameters AND native HiFi-GAN parameters,
to diagnose the source of audio degradation.

Three reconstructions compared:
  (A) Native HiFi-GAN  : 80-band mel, n_fft=1024, hop=256  (what HiFi-GAN was trained on)
  (B) Stylus HiFi-GAN  : 512-band mel, n_fft=2048, hop=512 → resampled to 80 bands before HiFi-GAN
  (C) Griffin-Lim      : reference baseline from Stylus mel
...
# ─────────────────────────────────────────────────────────────────────────────
# Stylus mel parameters (from StylusConfig)
# ─────────────────────────────────────────────────────────────────────────────
STYLUS_SR = 22050
STYLUS_N_FFT = 2048
STYLUS_HOP = 512
...
# Native HiFi-GAN mel parameters (LJ Speech / VCTK checkpoint)
HIFIGAN_SR = 22050
HIFIGAN_N_FFT = 1024
HIFIGAN_HOP = 256
HIFIGAN_WIN = 1024
...
def audio_metrics(ref, est, sr=22050):
    """
    Perceptual metrics adapted for audio reconstruction.

    Sample-to-sample SNR is useless for a neural vocoder:
    HiFi-GAN produces a phase-shifted signal, scale is arbitrary,
    and a slight length difference breaks everything.

    Used instead:
      - Spectral correlation (mel): compares frequency envelopes
      - Log-spectral distance: dB distance between time-averaged mels
      - Relative RMS energy: checks amplitude consistency
    """
    T = min(len(ref), len(est))
    ref, est = (
        ref[:T] / (np.abs(ref[:T]).max() + 1e-8),
        est[:T] / (np.abs(est[:T]).max() + 1e-8),
    )

    # Mel of both signals (80 bands, standard parameters)
    def mel(x):
...
# ─────────────────────────────────────────────────────────────────────────────
# Mel encode with Stylus parameters
# ─────────────────────────────────────────────────────────────────────────────


def encode_stylus_mel(audio):
    """audio → (mel_db, stft) with exact Stylus parameters."""
    stft = librosa.stft(
        audio, n_fft=STYLUS_N_FFT, hop_length=STYLUS_HOP, window="hann", center=True
    )
    mag = np.abs(stft)
...
# ─────────────────────────────────────────────────────────────────────────────
# Mel encode with native HiFi-GAN parameters
# ─────────────────────────────────────────────────────────────────────────────


def encode_hifigan_mel(audio):
    """audio → mel_db with native HiFi-GAN parameters."""
    mel = librosa.feature.melspectrogram(
        y=audio,
        sr=HIFIGAN_SR,
        n_fft=HIFIGAN_N_FFT,
        hop_length=HIFIGAN_HOP,
        win_length=HIFIGAN_WIN,
...
def resample_stylus_mel_to_hifigan(mel_db_stylus):
    ...
    t = F.interpolate(
        t, size=(n_bins_hifi, mag_stft.shape[1]), mode="bilinear", align_corners=False
    )
    ...
    t = F.interpolate(
        t, size=(HIFIGAN_N_MELS, T_target), mode="bilinear", align_corners=False
    )
...
def load_hifigan(device="cpu"):
    """
    Loads HiFi-GAN. Three attempts in order:
      1. NVIDIA torch.hub  — returns a tuple (model, denoiser), we take [0]
      2. SpeechBrain       — inference API
      3. Hugging Face      — microsoft/speecht5_hifigan (universal fallback)
    """
    ...
    try:
        ...
        # nvidia_hifigan returns (generator, denoiser) — we only want the generator
...
def encode_mel_for_backend(audio, backend, device="cpu"):
    """
    Encodes the audio with the NATIVE mel pipeline of each backend.
    We no longer go through our Stylus mel — each vocoder has its own
    encoder with its own normalization parameters.
    """
    if backend in ("nvidia", "speechbrain"):
        # Standard HiFi-GAN: n_mels=80, sr=22050, n_fft=1024, hop=256
        # Normalization: natural log (not librosa dB)
        mel = librosa.feature.melspectrogram(
            y=audio,
            sr=22050,
            n_fft=1024,
        ...
        # SpeechT5HifiGan is trained with the SpeechT5FeatureExtractor
        # sr=16000, n_mels=80, specific mean/var normalization
        ...
        # return_tensors="pt" → dict with "input_values" (mel features)
        feats = fe(
            audio=audio_16k, sampling_rate=16000, return_tensors="pt", padding=False
        )
        # feats contains the normalized log-mel spectrogram (1, T, 80)
...
def hifigan_decode(vocoder, backend, audio_raw, device="cpu"):
    """
    audio_raw: original numpy waveform (re-encoded with native pipeline)
    Returns  : audio numpy float32 reconstructed by the vocoder
    """
...
def griffinlim_from_stylus_mel(mel_db, stft_content=None):
    """
    Griffin-Lim reconstruction from Stylus mel.
    If stft_content is provided: initialised with content phase.
    """
    ...
    if stft_content is not None:
        ...
        for _ in range(8):
            ...
            audio_tmp = librosa.istft(
                s, n_fft=STYLUS_N_FFT, hop_length=STYLUS_HOP, window="hann", center=True
            )
    else:
...
def main():
    ...
    print(
        f"  mel_stylus   : {mel_stylus.shape}  ({mel_stylus.min():.1f} → {mel_stylus.max():.1f} dB)"
    )
    print(
        f"  mel_native   : {mel_native.shape}   ({mel_native.min():.1f} → {mel_native.max():.1f} dB)"
    )
    print(
        f"  mel_resampled: {mel_resampled.shape}  ({mel_resampled.min():.1f} → {mel_resampled.max():.1f} dB)"
    )
    ...
    if vocoder is not None:
        ...
        try:
            ...
            save_audio(
                os.path.join(OUT_DIR, "audio_A_hifigan_native.wav"), audio_A, HIFIGAN_SR
            )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Metriques (mel_corr: 1=parfait | lsd: 0=parfait | rms_diff: 0=meme volume)\n"
        )
        f.write("-" * 50 + "\n")
